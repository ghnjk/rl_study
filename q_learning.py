...
class QLearning(object):
    ...
    def learn(self, cur_state, action, next_state, reward, learning_rate = 0.01, reward_decent_rate = 0.9):
        predict_value = self.table.loc[cur_state, action]
        if next_state is None:
            target = reward
        else:
            self.check_state_exist(next_state)
            target = reward + reward_decent_rate * self.table.loc[next_state, :].max()
...
if __name__ == '__main__':
    ...
    ql = QLearning(env.get_all_actions())
    epoch_count = 1000
    avg_rate = []
    avg_len = []
    all_reward = 0
    ...
    for epoch in range(epoch_count):
        ...
        while not is_terminate:
            ...
            # 使用这种更新效率比较底下
            #ql.learn(s, action, ns, reward)
            step_list.append((s, action, ns, reward))
            s = ns
        for i in range(len(step_list) - 1, -1, -1):
            s, action, ns, _ = step_list[i]
            ql.learn(s, action, ns, reward / float(len(step_list) - i))
        all_reward += reward
