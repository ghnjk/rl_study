...
from memory import RandomMemory, SumTreeMemory
...
class PrioritizedDQN(object):
    def __init__(self, feature_cnt, action_cnt,
        ...
        if self.is_prioritized:
            self.memory = SumTreeMemory(self.memory_size)
        else:
            self.memory = RandomMemory(self.memory_size)
        self.learn_count = 0
        self.running_q = 0
    ...
    def build_net(self):
        ...
        with tf.variable_scope("loss"):
            self.abs_erros = tf.reduce_sum(tf.abs(self.q_target_in - self.q_eval), axis=1)
            if self.is_prioritized:
                self.loss = tf.reduce_sum(self.is_weights * tf.squared_difference(self.q_target_in, self.q_eval))
            else:
    ...
    def learn(self, show_msg=True):
        ...
        sample_weights = None
        sample_tree_idx = None
        sample_cnt = self.batch_size if self.batch_size < self.memory.data_count() else self.memory.data_count()
        if self.is_prioritized:
            sample_tree_idx, samples, sample_weights = self.memory.choose_sample(sample_cnt)
        else:
            samples = self.memory.choose_sample(sample_cnt)
        c_s = []
        act = []
        r = []
        n_s = []
        for i in range(len(samples)):
            c_s.append(samples[i][0])
            act.append(samples[i][1])
            r.append(samples[i][2])
            n_s.append(samples[i][3])
        # train
        ...
        # 训练evalute_net参数
        if self.is_prioritized:
            _, sample_abs_errors, cost = self.session.run(
                [self.train, self.abs_erros, self.loss],
                feed_dict={
                    self.cur_state_in: c_s,
                    self.q_target_in: q_target,
                    self.is_weights: sample_weights
                }
            )
            self.memory.batch_update(sample_tree_idx, sample_abs_errors)
        else:
